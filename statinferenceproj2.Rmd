...
output: pdf_document
---
In this document describes some exploratory data analyssi on the ToothGrowth dataset provided with the base R distribution, and checks relevant hypotheses based on the data.
...
We can quickly check the mean of the tooth growth by *supplement type* and *dose*:
```{r, comment=""}
...
We can also explore the results with some boxplots to show the relationships between the factors and the growth results:  
...
The *p-value* is greater than 0.05 and the confidence interval contains zero, therefore we we don't have enough statistical evidence to to reject the *null* hypothesis with 95% confidence.
...
We have three levels for the doses, so we will have to compare them in pairs. The *null* hypothesis will assume an equal mean across levels, and the alternative will be a difference.
